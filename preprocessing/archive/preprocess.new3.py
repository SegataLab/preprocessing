...
def quality_control(input_files, output_dir, dry_run=False, nproc=1):
    ...
    trimmed_R1 = os.path.join(output_dir, os.path.basename(input_R1).replace('.fastq', '_trimmed.fq'))
    trimmed_R2 = os.path.join(output_dir, os.path.basename(input_R2).replace('.fastq', '_trimmed.fq'))
    cmd = (
        f"fastp --in1 {input_R1} --in2 {input_R2} "
        "--detect_adapter_for_pe --average_qual 25 --qualified_quality_phred 20 --length_required 50 --n_base_limit 2 "
        "--trim_poly_g --trim_poly_x --low_complexity_filter --verbose "
        f"--out1 {trimmed_R1} --out2 {trimmed_R2}"
    )
    execute_command(cmd, dry_run)
    if not os.path.isfile(trimmed_R1) or not os.path.isfile(trimmed_R2):
